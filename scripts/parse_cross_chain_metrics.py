...
# Captures the rho value setting from Scenario B logs
RHO_SETTING_RE = re.compile(r'>>> Testing with rho = (?P<rho>\d+\.?\d*)% cross-chain <<<')
...
def parse_log_file(log_file_path):
    """
    Parses the Scenario B log file to extract approximate transaction timings, details, and rho value.
    ...
    rho_value = None            # Stores the parsed cross-chain ratio (0.0 to 1.0)
        ...
        with open(log_file_path, 'r') as f:
            for line in f:
                ...
                if rho_value is None:
                    match_rho = RHO_SETTING_RE.search(line)
                    if match_rho:
                        try:
                            rho_percent = float(match_rho.group('rho'))
                            rho_value = rho_percent / 100.0
                            print(f"  Detected Cross-Chain Ratio (rho): {rho_percent:.1f}%")
                        except ValueError:
                             print(f"  Warning: Could not parse rho value from line: {line.strip()}")
    ...
    if rho_value is None:
         print("  Warning: Could not detect cross-chain ratio (rho) setting in the log file.")

    print(f"\nParsing Complete: Read {line_count} lines.")
    # print(f"Found {len(pseudo_start_times)} unique TXs with embedded start_time (tv_sec).") # Removed, calculate after processing
    ...
    valid_interval_tx_ids = set() # Tx IDs for which we calculated an interval
    for i in range(len(sorted_confirm_list) - 1):
        ...
        if duration >= 0:
            ...
            pseudo_start_times[tx_id_curr] = (sec_curr, nsec_curr) # Store original start time for reference
            transaction_details[tx_id_curr] = details_curr # Store details
        else:
    ...
    if not pseudo_completion_times:
        print("\nWARNING: No pseudo-completion times (proposal_time_since_epoch from AppendEntries) could be extracted. Throughput calculation impossible.")

    return rho_value, inter_submission_intervals, pseudo_completion_times, transaction_details, valid_interval_tx_ids, len(submit_result_tx_ids)


def analyze_results(rho_value, inter_submission_intervals, pseudo_completion_times, transaction_details, valid_tx_ids, submit_result_count):
    """
    Calculates and prints APPROXIMATE throughput and interval metrics for Scenario B based on EMBEDDED log times.
    WARNING: Results derived from this function have SIGNIFICANT ACCURACY LIMITATIONS.
    """
    rho_string = f"{rho_value * 100:.1f}%" if rho_value is not None else "Unknown"
    print(f"\n--- Analysis Results (Scenario B, rho = {rho_string}) (APPROXIMATE - Based on Embedded Log Times) ---")
    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    ...
    if not pseudo_completion_times:
        ...
        if num_total_completed_tx > 0:
            ...
            if not valid_completion_times:
                 print("  - Average TPS: Error - No valid float completion time values found.")
                ...
                if processing_duration > 0:
                    ...
                    print(f"  - Average TPS: Infinite? ({len(valid_completion_times)} transactions proposed in effectively zero time)")
    ...
    print(f"\n--- Approximate Inter-Submission Interval Analysis (rho = {rho_string}) ---")
    if not valid_tx_ids:
        print("❌ APPROXIMATE INTERVALS CANNOT BE CALCULATED.")
...
def analyze_intervals(intervals_data, transaction_details, valid_tx_ids):
    ...
    for tx_id in valid_tx_ids:
        ...
        if interval is not None and isinstance(interval, float) and interval >= 0:
            ...
            if details:
                try:
                    # Define cross-chain: source != target AND target is not 0 (assuming 0 is a special value, e.g., mint/burn)
            ...
            # Includes tx_ids in valid_tx_ids but missing from intervals_data, or bad interval value
            skipped_missing_data += 1

    if skipped_missing_data > 0:
        print(f"  Warning: Skipped {skipped_missing_data} intervals due to missing data or negative values.")
    ...
    if intervals:
        ...
        # Handle cases with insufficient data for quantiles gracefully
        p95_interval = float('nan')
        p99_interval = float('nan')
        if len(intervals) >= 20: # Need at least 20 for P95 (index 19 of 0-19 for 5%ile, index 94 for 95%ile)
            try:
                p95_interval = statistics.quantiles(intervals, n=100)[94]
            except statistics.StatisticsError:
                print("  Warning: Could not calculate P95 quantile.")
        if len(intervals) >= 100: # Need 100 for P99 (index 98)
             try:
                p99_interval = statistics.quantiles(intervals, n=100)[98]
             except statistics.StatisticsError:
                print("  Warning: Could not calculate P99 quantile.")
        ...
        print(f"  - P95:     {p95_interval:.4f} seconds" if not math.isnan(p95_interval) else "  - P95:     N/A (Insufficient data or error)")
        print(f"  - P99:     {p99_interval:.4f} seconds" if not math.isnan(p99_interval) else "  - P99:     N/A (Insufficient data or error)")
    ...
    if cross_chain_intervals:
        ...
        min_cc_interval = min(cross_chain_intervals)
        max_cc_interval = max(cross_chain_intervals)
        ...
        print(f"  - Min:     {min_cc_interval:.4f} seconds")
        print(f"  - Max:     {max_cc_interval:.4f} seconds")
    else:
    ...
    if intra_chain_intervals:
        ...
        min_ic_interval = min(intra_chain_intervals)
        max_ic_interval = max(intra_chain_intervals)
        ...
        print(f"  - Min:     {min_ic_interval:.4f} seconds")
        print(f"  - Max:     {max_ic_interval:.4f} seconds")
    else:
        print("\nNo intra-chain transactions with valid interval data found.")


def main():
    parser = argparse.ArgumentParser(description="Analyze Scenario B (Cross-Chain Ratio) log files using embedded time fields (APPROXIMATE RESULTS).")
    parser.add_argument("log_file", help="Path to the Scenario B output log file")
    args = parser.parse_args()

    # Parse log file using embedded times
    rho_value, inter_submission_intervals, pseudo_completion_times, transaction_details, valid_tx_ids, submit_result_count = parse_log_file(args.log_file)

    # Proceed with analysis if parsing found *some* data, even if incomplete
    analyze_results(rho_value, inter_submission_intervals, pseudo_completion_times, transaction_details, valid_tx_ids, submit_result_count)
